...
int KernelCvarWait(int cvar_id, int lock_id){
    q_t *cvarQ = getCvar(cvar_id);
     if (cvarQ == NULL) {
        TracePrintf(1, "KernelCvarWait: getCvar(%d) returned NULL\n", cvar_id);
        return ERROR;
    }
...
int KernelCvarSignal(int cvar_id){
	q_t *cvarQ = getCvar(cvar_id);
    if (cvarQ == NULL) {
        TracePrintf(1, "KernelCvarSignal: getCvar(%d) returned NULL\n", cvar_id);
        return ERROR;
    }
...
int KernelCvarBroadcast(int cvar_id){
	q_t *cvarQ = getCvar(cvar_id);
    if (cvarQ == NULL) {
        TracePrintf(1, "KernelCvarBroadcast: getCvar(%d) returned NULL\n", cvar_id);
        return ERROR;
    }
